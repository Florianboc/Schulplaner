...
async function loadGroup() {
  ...
  // Button-Click überschreiben → immer richtigen Code kopieren
  ...
  box.innerHTML = data
    .map(
      ...
      <div class="event-item">
        ...
        e.title
      )}', '${e.date}', \`${e.description ? e.description.replace(/`/g, "\\`") : ""}\`)">
            ✏️ Bearbeiten
          </button>
...
async function renderSinglePoll(poll) {
    ...
    const myVote = votes.find(v => v.user_id === session.user.id)?.option_id;
    ...
    class="${votes.some(v => v.user_id === session.user.id && v.option_id === opt.id) ? "voted" : ""}"
                    onclick="votePoll('${poll.id}', '${opt.id}')"
                    class="${myVote === opt.id ? "voted" : ""}">
                    ${opt.text}
                </button>
...
async function votePoll(pollId, optionId) {
    // Prüfen ob User DIESE Option schon gewählt hat
...
async function loadTaskAssignees() {
  ...
  // Filter-Assignee hat schon all/me, wir hängen Personen drunter
  ...
  data.forEach((m) => {
    ...
    // Filter-Select nur ergänzen, wenn nicht schon da
...
async function loadTasksKanban() {
  ...
  tasks.forEach((task) => {
    ...
    el.classList.add("task-card", "fade-in");
...
// =========================================================
// TASK-KOMMENTARE (pro Aufgabe eigener Bereich)
// =========================================================
async function openTaskComments(taskId) {
  ...
  if (error) {
    console.warn("task_comments fehlen evtl. in DB:", error.message);
...
async function addTaskComment(taskId) {
  ...
  if (error) {
    console.warn("Fehler beim Speichern von task_comments:", error.message);
    alert("Kommentar konnte nicht gespeichert werden (DB-Tabelle vorhanden?).");
    return;
  }

  input.value = "";
  await logActivity("task_comment", `Kommentar zu Aufgabe ${taskId}`);
...
// =============================================
//   Aufgaben (inkl. Fortschrittsbalken)
// =============================================
async function loadOverviewTasks() {
    ...
    document.getElementById("ov-tasks").innerHTML = `
        Offen: ${data.filter(t => t.status === "open").length}<br>
        In Arbeit: ${data.filter(t => t.status === "in_progress").length}<br>
        Erledigt: ${done}
    `;

    document.getElementById("ov-tasks-fill").style.width = progress + "%";
}
...
async function loadOverviewMoney() {
    ...
    let total = 0;
    data.forEach(e => {
        if (e.category === "ausgabe") total -= e.amount;
        else total += e.amount;
    });
...
async function loadOverviewActivity() {
    ...
    box.innerHTML = data.map(a => `
        <div class="activity-item">
            <small>${new Date(a.created_at).toLocaleString("de-DE")}</small>
            <br>
            ${a.message}
        </div>
    `).join("");
...
// Einfacher Logger – falls Tabelle fehlt, wird nur in der Konsole gemeckert
